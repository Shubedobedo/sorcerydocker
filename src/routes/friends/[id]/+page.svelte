      ...
      {#if data.canSeeCollection}
        {#if data.sharedCollection.length > 0}
          <p class="collection-stats">
            {data.sharedCollection.length} unique cards · {data.sharedCollection.reduce(
              (s, i) => s + i.quantity,
              0
            )} total
          </p>
      ...
      {#if data.canSeeTrades}
        {#if data.sharedTrades.length > 0}
          {#if filteredTradeValue > 0}
            <p class="collection-stats">
              {filteredTrades().length}{#if filteredTrades().length !== data.sharedTrades.length}
                of {data.sharedTrades.length}{/if} cards
              <span class="section-value"
                >&middot; ${filteredTradeValue.toFixed(2)} market value</span
              >
            </p>
          {/if}
          <div class="filters-bar">
            ...
            <div class="segmented-control">
              <button
                class="seg-btn"
                class:active={tradeFilters.foil === ''}
                onclick={() => {
                  tradeFilters.foil = '';
                }}>All</button
              >
              <button
                class="seg-btn"
                class:active={tradeFilters.foil === 'nonfoil'}
                onclick={() => {
                  tradeFilters.foil = 'nonfoil';
                }}>Non-Foil</button
              >
              <button
                class="seg-btn"
                class:active={tradeFilters.foil === 'foil'}
                onclick={() => {
                  tradeFilters.foil = 'foil';
                }}>Foil</button
              >
            </div>
                  ...
                  <div class="multi-select-dropdown">
                    {#each availableSetOptions() as setName}
                      <label class="checkbox-item">
                        <input
                          type="checkbox"
                          checked={tradeFilters.sets.includes(setName)}
            ...
            <div class="segmented-control">
              <button
                class="seg-btn"
                class:active={tradeFilters.price === ''}
                onclick={() => {
                  tradeFilters.price = '';
                }}>All</button
              >
              <button
                class="seg-btn"
                class:active={tradeFilters.price === 'under1'}
                onclick={() => {
                  tradeFilters.price = 'under1';
                }}>&lt; $1</button
              >
              <button
                class="seg-btn"
                class:active={tradeFilters.price === '1to5'}
                onclick={() => {
                  tradeFilters.price = '1to5';
                }}>$1&ndash;5</button
              >
              <button
                class="seg-btn"
                class:active={tradeFilters.price === '5to20'}
                onclick={() => {
                  tradeFilters.price = '5to20';
                }}>$5&ndash;20</button
              >
              <button
                class="seg-btn"
                class:active={tradeFilters.price === '20plus'}
                onclick={() => {
                  tradeFilters.price = '20plus';
                }}>$20+</button
              >
            </div>
                    ...
                    {#if trade.price != null}
                      <span class="trade-market"
                        >Market: ${trade.price.toFixed(2)}{#if trade.quantity > 1}
                          · ${(trade.price * trade.quantity).toFixed(2)} total{/if}{#if trade.foil}
                          (foil){/if}</span
                      >
                    {/if}
